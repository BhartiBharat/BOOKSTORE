...
const Sidebar = ({data}) => {
  ...
  return (
    <div className=' bg-zinc-800 py-4 rounded flex flex-col items-center justify-between h-auto lg:h-[100%]'>
...
<p className='mt-3 text-xl text-zinc-100 font-semibold'>
    ...
    {role==="user" && (<div className='w-full flex-col items-center justify-center hidden lg:flex'>
        <Link to="/profile"
        className="text-zinc-100 font-semibold w-full py-2 text-center hover:bg-zinc-900 rounded 
        transition-all duration-300">
            Favourite
        </Link>
        <Link to="/profile/orderHistory"
        className="text-zinc-100 font-semibold w-full py-2 mt-4 text-center hover:bg-zinc-900 rounded 
        transition-all duration-300">
            order History
        </Link>
    ...
    py-2 rounded hover:bg-white hover:text-zinc-900 transition-all duration-300'
    onClick={()=>{
    ...
    Log Out <FaAngleRight className="ms-4"/>
    </button>
    </div>
  )
}
